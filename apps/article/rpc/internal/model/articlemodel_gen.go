...
var (
	...
	cacheMyzhihuArticleArticleIdPrefix = "cache:myzhihuArticle:article:id:"
	// cacheBeyondArticleArticleIdPrefix = "cache:beyondArticle:article:id:" 这个是up主的 和我的名字不一样
)
...
func (m *defaultArticleModel) Delete(ctx context.Context, id int64) error {
	myzhihuArticleArticleIdKey := fmt.Sprintf("%s%v", cacheMyzhihuArticleArticleIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, myzhihuArticleArticleIdKey)
	return err
}
...
func (m *defaultArticleModel) FindOne(ctx context.Context, id int64) (*Article, error) {
	myzhihuArticleArticleIdKey := fmt.Sprintf("%s%v", cacheMyzhihuArticleArticleIdPrefix, id)
	var resp Article
	err := m.QueryRowCtx(ctx, &resp, myzhihuArticleArticleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultArticleModel) Insert(ctx context.Context, data *Article) (sql.Result, error) {
	myzhihuArticleArticleIdKey := fmt.Sprintf("%s%v", cacheMyzhihuArticleArticleIdPrefix, data.Id)
...
func (m *defaultArticleModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheMyzhihuArticleArticleIdPrefix, primary)
}
